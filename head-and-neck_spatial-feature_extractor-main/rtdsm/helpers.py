...
def find_plane_intersection(n1,p1,n2,p2,zeroind=0):
    ...
    if np.array_equal(a[:,0],[0,0]) or np.array_equal(a[:,1],[0,0]):
        print('WARNING!!! System of equations forms a singular matrix. Cannot solve for the line of intersection. Try using a zeroind value other than',zeroind)
        print(a,'<--Problem Matrix')
        raise Exception(np.linalg.solve(a,b))
